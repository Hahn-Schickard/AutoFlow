...
def main_functions(project_dir, model_name, model_input_neurons,
    ...
    Args:
        ...
        model_input_neurons:    Number of neurons in the input layer of the
                                model
        model_output_neurons:   Number of neurons in the output layer of the
                                model
        quant_dtype:            Data type to quantize to
        model_memory:           Preallocate a certain amount of memory for
                                input, output, and intermediate arrays in
                                kilobytes
    """
    ...
    with open(project_dir + "/src/TF_Lite_exe.cpp", "w") as f:
        ...
        f.write('#include "TF_Lite_exe.h"\n'
                ...
                'intermediate arrays.\n'
                'constexpr int kTensorArenaSize = ' + str(model_memory)
                + ' * 1024;\n'
        ...
        if model_output_neurons == 1:
            ...
            if quant_dtype is not None and "int8 only" in quant_dtype:
                f.write('int8_t* prediction = new int8_t['
                        + str(model_output_neurons) + '];\n')
            else:
                f.write('float* prediction = new float['
                        + str(model_output_neurons) + '];\n')
        f.write('}\n'
                '\n'
                'void setup_model() {\n'
                ...
                "tensors.\n"
                '  TfLiteStatus allocate_status = interpreter->'
                'AllocateTensors();\n'
        ...
        if model_output_neurons == 1:
            if quant_dtype is not None and "int8 only" in quant_dtype:
                f.write('int8_t model_execute(int8_t *input_data) {\n'
                        '  for (int i = 0; i < ' + str(model_input_neurons)
                        + '; ++i) {\n'
        ...
        if model_output_neurons == 1:
            ...
            f.write('  for (int i = 0; i < ' + str(model_output_neurons)
                    + '; i++) {\n')
            if quant_dtype is not None and "int8 only" in quant_dtype:
