...
class UIProjectData(QWidget):
    ...
    def __init__(self, WINDOW_WIDTH, WINDOW_HEIGHT, FONT_STYLE, parent=None):
        super(UIProjectData, self).__init__(parent)
        ...
        self.output_path_Browse.setToolTip(
            'Select a path where the AutoFlow\n'
        ...
        self.keras_model_label.setStyleSheet(
            "font: " + str(int(0.035 * self.WINDOW_HEIGHT)) + "px " +
            FONT_STYLE)
        ...
        model_img = QPixmap(os.path.join(
        ...
        self.select_model_browse.setToolTip(
            'Select a TensorFlow/Keras model (.h5 file)\n'
            'which should be converted.')
        self.select_model_browse.setStyleSheet(
            """QPushButton {
            font: """ + str(int(0.035 * self.WINDOW_HEIGHT)) + """px """ + FONT_STYLE + """}
            QPushButton::hover {
            background-color : rgb(10, 100, 200)}
        ...
        self.horizontal_box.append(QHBoxLayout())
        self.horizontal_box[8].addItem(QSpacerItem(
            0.03 * self.WINDOW_WIDTH, 0.03 * self.WINDOW_HEIGHT))
        ...
        self.horizontal_box[11].addItem(QSpacerItem(
            0.08 * self.WINDOW_WIDTH, 0.08 * self.WINDOW_HEIGHT))
