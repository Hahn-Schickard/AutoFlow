...
class UIAutoMLTraining(QWidget):
    ...
    """
    def __init__(self, WINDOW_WIDTH, WINDOW_HEIGHT, FONT_STYLE, project_name,
                 output_path, data_path, task, max_trials, max_epochs,
                 max_size, num_channels, img_height, img_width, separator,
                 decimal, csv_target_label, parent=None):
        ...
        self.autokeras = Autokeras(
            self.project_name, self.output_path, self.data_path, self.task,
            self.max_trials, self.max_epochs, self.max_size, self.num_channels,
            self.img_height, self.img_width, self.separator, self.decimal,
