...
def OptiWindow(self):
    ...
    self.Window3 = UIOptiWindow(self.WINDOW_WIDTH, self.WINDOW_HEIGHT,
                                self.FONT_STYLE, self.target, self)
    ...
    self.Window3.pruning.toggled.connect(
        lambda: self.set_pruning(self.Window3))
    self.Window3.quantization.toggled.connect(
        lambda: self.set_quantization(self.Window3))

    self.Window3.prun_fac.clicked.connect(
        lambda: self.set_prun_type("Factor", self.Window3, False))
    self.Window3.prun_acc.clicked.connect(
        lambda: self.set_prun_type("Accuracy", self.Window3, False))

    self.Window3.min_acc.clicked.connect(
        lambda: self.set_prun_acc_type("Minimal accuracy", self.Window3))
    self.Window3.acc_loss.clicked.connect(
        lambda: self.set_prun_acc_type("Accuracy loss", self.Window3))

    self.Window3.quant_int.clicked.connect(
        lambda: self.set_quant_dtype("int8 with float fallback", self.Window3))
    self.Window3.quant_int_only.clicked.connect(
        lambda: self.set_quant_dtype("int8 only", self.Window3))
    ...
    if n == "Back":
            ...
            try:
                if "Factor" in self.prun_type:
                    self.prun_factor_dense = (int(
                        self.Window3.pruning_dense.text()))
                    self.prun_factor_conv = (int(
                        self.Window3.pruning_conv.text()))
        ...
        if "Pruning" in self.optimizations:
            if self.prun_type is None:
                ...
                try:
                    if (int(self.Window3.pruning_dense.text()) > 95 or
                            int(self.Window3.pruning_conv.text()) > 95):
                        msg = QMessageBox()
                        msg.setIcon(QMessageBox.Warning)
                ...
                try:
                    ...
                    if "Minimal accuracy" in self.prun_acc_type:
                        if (int(self.Window3.prun_acc_edit.text()) <= 50 or
                                int(self.Window3.prun_acc_edit.text()) > 99):
                            msg = QMessageBox()
                            msg.setIcon(QMessageBox.Warning)
                        ...
                        if (int(self.Window3.prun_acc_edit.text()) < 1 or
                                int(self.Window3.prun_acc_edit.text()) > 20):
                            msg = QMessageBox()
                            msg.setIcon(QMessageBox.Warning)
