...
def LoadWindow(self):
    ...
    self.Window5.back.clicked.connect(
        lambda: nextWindow(self, "Back", self.optimizations, self.Window5))

    self.Window5.load.clicked.connect(
        lambda: nextWindow(self, "Next", self.optimizations, self.Window5))

    self.Window5.prune_model.request_signal.connect(
        lambda: self.convert_create(self.Window5))
    self.Window5.conv_build_load.request_signal.connect(
        lambda: self.terminate_thread(self.Window5))
...
def setWindowLabels(self, CurWindow):
    ...
    """
    self.set_label(CurWindow.project_name_label_2, self.project_name,
                   Qt.AlignLeft)
    self.set_label(CurWindow.output_path_label_2, self.output_path,
                   Qt.AlignLeft)
    ...
    if ("Pruning" in self.optimizations and
        ...
        self.set_label(CurWindow.optimizations_label_2,
                       "Pruning + Quantization", Qt.AlignLeft)
    elif len(self.optimizations) != 0:
        self.set_label(CurWindow.optimizations_label_2, self.optimizations[0],
                       Qt.AlignLeft)
    else:
    ...
    if "Pruning" in self.optimizations:
        if "Factor" in self.prun_type:
            self.set_label(
                CurWindow.pruning_label_2, "Pruningfactor dense: " +
            ...
            if "Minimal accuracy" in self.prun_acc_type:
                self.set_label(
                    CurWindow.pruning_label_2, "Minimal accuracy to reach: " +
                    str(self.prun_acc) + "%", Qt.AlignLeft)
            else:
                self.set_label(
                    CurWindow.pruning_label_2, "Maximal accuracy loss: " +
                    str(self.prun_acc) + "%", Qt.AlignLeft)
    if "Quantization" in self.optimizations:
        if "int8 only" in self.quant_dtype:
            self.set_label(CurWindow.quantization_label_2, "Int8 only",
                           Qt.AlignLeft)
        else:
            self.set_label(CurWindow.quantization_label_2,
                           "Int8 with float32 fallback", Qt.AlignLeft)
    self.set_label(CurWindow.data_loader_label_2, self.data_loader_path,
                   Qt.AlignLeft)
...
def nextWindow(self, n, optimizations, CurWindow):
    ...
    if n == "Back":
        ...
        if "MCU" in self.target:
            try:
                if (int(CurWindow.model_memory.text()) < 5 or
                        int(CurWindow.model_memory.text()) > 1000):
                    msg = QMessageBox()
