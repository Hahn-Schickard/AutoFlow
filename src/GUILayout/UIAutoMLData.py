...
class UIAutoMLData(QWidget):
    ...
    def __init__(self, WINDOW_WIDTH, WINDOW_HEIGHT, FONT_STYLE, parent=None):
        ...
        self.output_path_browse.setStyleSheet(
            """QPushButton {
        ...
        data_img = QPixmap(os.path.join(
            'src', 'GUILayout', 'Images', 'Database.png'))
        ...
        self.dataloader_list.addItems(
            ["Select PATH with data", "Select FILE with data"])
        self.dataloader_list.setStyleSheet(
            "font: " + str(int(0.023 * self.WINDOW_HEIGHT)) + "px " +
            FONT_STYLE)
        ...
        self.select_data_browse.setToolTip(
            'Select the training data which the neural network requires for '
            'the optimization.\n'
            'The data can be transferred in different ways:\n'
            '- PATH (folder):\tImages are to be used as training data. In the '
            'given path there are\n'
            '\t\tsubfolders containing the name of the different classes of '
            'the neural\n'
            '\t\tnetwork and the corresponding images.\n'
            '- FILE (*.csv):\tThe data is stored in a CSV file.\n'
            '- FILE (*.py):\tThe data is loaded and returned in a Python '
            'script. It\n'
            '\t\tis important that the Python script contains the function\n'
            '\t\tget_data() with the return values x_train, y_train, x_test,\n'
            '\t\ty_test (training data, training label, test data, '
            'test label).\n'
            '\t\tThe return values here are Numpy arrays.')
        self.select_data_browse.setStyleSheet(
            """QPushButton {
            font: """ + str(int(0.035 * self.WINDOW_HEIGHT)) + """px """ + FONT_STYLE + """}
            QPushButton::hover {
            background-color : rgb(10, 100, 200)}
        ...
        self.horizontal_box[2].addItem(QSpacerItem(
            0.03 * self.WINDOW_WIDTH, 0.03 * self.WINDOW_HEIGHT))
        ...
        self.horizontal_box.append(QHBoxLayout())
        self.horizontal_box[11].addItem(QSpacerItem(
            0.03 * self.WINDOW_WIDTH, 0.03 * self.WINDOW_HEIGHT))
